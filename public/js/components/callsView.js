...
export function renderCalls(container) {
  ...
  container.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px"><h1 style="font-size:16px;font-weight:600;display:flex;align-items:center;gap:8px">Calls <span style="font-size:13px;color:var(--muted);font-weight:400">' + items.length + ' calls</span></h1></div><div class="upload-zone" id="calls-upload-zone" style="margin-bottom:16px"><div class="upload-zone-icon" style="font-size:14px;color:var(--muted)">CSV</div><div class="upload-zone-text"><h3>Upload AT&T Call Log</h3><p>CSV format</p></div><button class="btn btn-ghost btn-sm" style="margin-left:auto" onclick="event.stopPropagation();this.closest(\'.upload-zone\').querySelector(\'input\').click()">Choose File</button><input type="file" accept=".csv" style="display:none"></div><div id="calls-table-container"></div>';
