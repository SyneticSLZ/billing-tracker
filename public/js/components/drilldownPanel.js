...
function ensurePanel() {
  ...
  panelEl.innerHTML = '<div class="drilldown-header"><h3 id="drilldown-title"></h3><button class="drilldown-close">&times;</button></div><div class="drilldown-body" id="drilldown-body"></div><div class="drilldown-footer" id="drilldown-footer"></div>';
...
export function open(type, item, onEdit) {
  ...
  if (item.client) html += '<div class="detail-section"><div class="detail-label">Client</div><div class="detail-value" style="font-weight:600;color:' + (item.client.includes('UNKNOWN') ? 'var(--warning)' : 'var(--accent)') + '">' + escapeHtml(item.client) + (item.rmKeyMatched ? ' <span style="font-size:11px;color:var(--success)">matched</span>' : '') + '</div></div>';
