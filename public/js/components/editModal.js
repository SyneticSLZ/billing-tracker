...
function ensureModal() {
  if (modalEl) return;
  modalEl = document.getElementById('edit-modal');
  if (!modalEl) return;
...
export async function saveEdit() {
  ...
  const updates = {
    ...
    activityDescription: document.getElementById('edit-description').value,
    date: document.getElementById('edit-date').value,
    durationHours: parseFloat(document.getElementById('edit-duration').value) || 0.1,
    startFormatted: document.getElementById('edit-start').value,
    endFormatted: document.getElementById('edit-end').value,
  };

  try {
    const data = await updateEntry(id, updates);
    if (data.success) {
      const idx = store.billingItems.findIndex(i => i.id === id);
      if (idx !== -1) store.billingItems[idx] = { ...store.billingItems[idx], ...updates };
      closeModal();
      toast('Entry updated');
...
// Make save/close available globally for onclick handlers
