...
export class DataTable {
  ...
  render() {
    ...
    // Toolbar
    html += '<div class="data-table-toolbar"><div class="toolbar-left">';
    if (this.showSearch) {
      html += `<input type="text" class="table-search" placeholder="Search..." value="${escapeHtml(this.searchQuery)}" data-action="search">`;
    }
    this.filters.forEach(f => {
      html += `<select class="table-filter" data-filter-key="${f.key}">
        <option value="">All ${f.label}</option>
        ${f.options.map(o => `<option value="${escapeHtml(o)}" ${this.filterValues[f.key] === o ? 'selected' : ''}>${escapeHtml(o)}</option>`).join('')}
      </select>`;
    });
    html += '</div><div class="toolbar-right">';
    this.actions.forEach(a => {
      html += `<button class="btn ${a.class || 'btn-ghost btn-sm'}" data-table-action="${a.id}">${a.label}</button>`;
    });
    html += `<span style="font-size:0.72rem;color:var(--muted);font-family:'DM Mono',monospace">${filtered.length} items</span>`;
    html += '</div></div>';
    ...
    this.columns.forEach(col => {
      ...
      if (col.key === this.sortColumn) {
        sortClass = this.sortDir === 'asc' ? 'sort-asc' : 'sort-desc';
    ...
    if (!pageItems.length) {
      ...
      pageItems.forEach(item => {
        ...
        this.columns.forEach(col => {
          const cls = col.class || '';
          const val = col.render ? col.render(item) : escapeHtml(String(item[col.key] ?? ''));
    ...
    if (filtered.length > this.pageSize) {
      ...
      const showEnd = Math.min(start + this.pageSize, filtered.length);
      html += `<div class="table-pagination">
        <span class="pagination-info">Showing ${showStart}-${showEnd} of ${filtered.length}</span>
        <div class="pagination-controls">
          <button class="pagination-btn" data-page="prev" ${this.page <= 1 ? 'disabled' : ''}>Prev</button>`;
      ...
      html += `<button class="pagination-btn" data-page="next" ${this.page >= totalPages ? 'disabled' : ''}>Next</button>
        </div></div>`;
    }
  ...
  _bindEvents() {
    ...
    // Filters
    this.container.querySelectorAll('.table-filter').forEach(select => {
      select.addEventListener('change', (e) => {
        this.filterValues[e.target.dataset.filterKey] = e.target.value;
        this.page = 1;
        this.render();
      });
    });
    ...
    if (this.onRowClick) {
      this.container.querySelectorAll('tbody tr[data-id]').forEach(tr => {
        tr.addEventListener('click', (e) => {
          ...
          const item = this.data.find(i => String(i.id) === id);
    ...
    // Actions
    this.container.querySelectorAll('[data-table-action]').forEach(btn => {
      btn.addEventListener('click', () => {
        if (this.onAction) this.onAction(btn.dataset.tableAction);
      });
    });
  }
}
